...
\begin{tabularx}{\linewidth}{ | l | X | }
  ...
  \hline
  	...
  	slotManufacturerId					& \\
  	timestamp			& Unix timestamp (seconds since epoch) \\
  	chargingState		& See Section \ref{types:ChargingState} \\	
  	meterValue & \\
  	battery/soc & percentage points \\
  	battery/temperature &  \\
  	battery/cycleCount &  \\
  	battery/voltage &  \\
  	battery/current &  \\
...
\begin{tabularx}{\linewidth}{ | l | X | }
  ...
  Field & Description \\
  \hline
  	status			& Upload status of the logs; see Section \ref{types:LogUpdateState} \\	
  \hline
\end{tabularx}
...
\begin{tabularx}{\linewidth}{ | l | X | }
  ...
  \hline
  	cardId 		& Card specific number\\
  	remainingTrials & Count of remaining authentication trials, before the account gets disabled.\\
    \hline
\end{tabularx}
\end{table}
...
\subsection{Validate Activation-Key}

\marginlabel{Description:}
Before a user can use his card (e.g., Bluecard) to rent a bike, he has to activate it on the \acs{PS} terminal. For this purpose, he type in an Activation-Key which gets validated from the \acs{CMS}. After the key got accepted, "Activate Card" follows.

The response shall indicate, whether or not the Activation-Key is accepted by the \acs{CMS}.

\marginlabel{URL:} \url{[BASE_CMS_URI]/validate-activation-key}
...
\begin{tabularx}{\linewidth}{ | l | X | }
  ...
  Field & Description \\
  \hline
  	activationKey & Key to start activation process to initial user's card\\
    \hline
\end{tabularx}
\end{table}
