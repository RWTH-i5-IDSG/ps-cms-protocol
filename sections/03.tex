...
\marginlabel{URL:} \url{[BASE_PS_URI]/pedelec-config/<pedelecManufacturerId>}
...
\marginlabel{Description:}
The CMS can send the PS a firmware update command. With a firmware update request, the CMS informs the PS about a new firmware, including the location of the firmware and the date \& time when the update shall be executed.

The PS should start as soon as possible the firmware after retrieving the retrieve-update.

\marginlabel{URL:} \url{[BASE_CMS_URI]/update-firmware}
...
\begin{tabularx}{\linewidth}{ | l | X | }
  ...
  executionDateTime	&  Date and time when the PS should execute update \\
  \hline
\end{tabularx}
